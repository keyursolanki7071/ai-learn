...
async def get_chat_history(thread_id: str):
    """Fetch formatted chat history from the SqliteSaver for the given thread_id."""
    async with AsyncSqliteSaver.from_conn_string("checkpoints.sqlite") as memory:
        agent = graph_builder.compile(checkpointer=memory, interrupt_before=["tools"])
        ...
        for msg in state.values["messages"]:
            ...
            # ToolMessages or AI tool calls might exist, we can handle them gracefully.
            ...
            history.append({
                ...
                "role": role,
                "content": content,
                "timestamp": None # LangGraph doesn't store timestamps natively by default
            })
